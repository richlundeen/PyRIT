...
_RESERVED_CONVERTER_NAMES = frozenset({"types"})
...
class ConverterService:
    ...
    async def create_converter_async(self, *, request: CreateConverterRequest) -> ConverterInstance:
        ...
        Raises:
            ValueError: If the converter type is not found, the name is reserved,
        ...
        if request.name in _RESERVED_CONVERTER_NAMES:
            raise ValueError(f"Converter instance name '{request.name}' is reserved")
        if request.name in self._registry.instances:
            raise ValueError(f"Converter instance '{request.name}' already exists")
        ...
        self._registry.instances.register(converter_obj, name=request.name)

        return self._build_instance_from_object(converter_id=request.name, converter_obj=converter_obj)
